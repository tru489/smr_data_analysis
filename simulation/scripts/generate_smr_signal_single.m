function generate_smr_signal_single(...
...
total_datapoints = (fwd_datapoints) * n_particles;
total_time = (fwd_search_time) * n_particles;
fwd_idx_start_datapoints = 1 + (0:n_particles - 1) * (fwd_datapoints);
...
% fwd_vstate = 11;
back_vstate = 7;
valve_state = ones(total_datapoints, 1) * back_vstate;
...
fwd_bl_fit_abc = zeros(n_particles, size(fwd_pkfit_params, 1));
local_curv_fwd_back = zeros(n_particles, 1);
buoy_mass_fwd_back = zeros(n_particles, 1);
dry_mass_arr = zeros(n_particles, 1);
dry_vol_arr = zeros(n_particles, 1);
tot_vol_arr = zeros(n_particles, 1);
rel_wc_arr = zeros(n_particles, 1);
arrival_frac_fwd_back = zeros(n_particles, 1);
pk_width_fwd_back = zeros(n_particles, 1);
...
gt_table = table();
gt_table.particle_idx = (1:n_particles)';
