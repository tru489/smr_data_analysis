...
%%
if nargin ==0
    % If no args, then just do cleanup
    clear 
    clc
    close all
else
    % If an arg was passed, this is the dir where the the data should be
    % read in from
...
global datafull % Stores all (13) features from all detected SMR signals
global x
global samplepeak;
global sampletime;
...
if nargin==0
    ...
    % Grab sample name from input file name for annotating output file name
    name_split = strsplit(input_info.smr_dir,'\');
    sample_name = name_split{end-1};
    sample_name = strrep(sample_name,'_',' ');
%=================================================================%
else
    % If data dir was specified:
    % Load frequency data based on SMR directory input
    %fprintf('\nGetting SMR data...\n')
    ...
    % Load time data based on SMR directory input
    %fprintf('\nGetting Time data...\n')
    ...
    % Grab sample name from input file name for annotating output filename
    name_split = strsplit(input_info.smr_dir,'\');
    sample_name = name_split{end-1};
    sample_name= strrep(sample_name,'_',' ');
end
...
% setting that created the raw data (ie PLL or Feedback mode), only part 
% of this code block is used while the other part is commented out
...
% Read time file. Removing the first element in the time file to initialize
% the rawdata_smr_time array to be the same size of rawdata_smr
...
% User inputs to choose analysis and display mode, rapid analysis (if yes) 
% means the pipeline will go through the entire dataset without seeking 
% user approval for passing an identified smr signal. Displaying progress 
% (if yes) will display individual detected peak shapes as well as all 
% detected peaks from the whole data block under analysis (data block size
% defined by "datasize" variable)
...
if nargin ==0
    ...
    % Only use analysis mode if data directory was input
    analysismode = 1;
    dispprogress = 0;
end
...
% Initialize estimated number of datapoints want to test. Number of
% datapoints corresponds with the size of the window within which each SMR
% signal should be captured. Idea is to choose the correct window size such
% that the maximum number of signals are captured; window too small or too
% large results in rejection of signals that have broader or more narrow
% peaks
estimated_datapoints = 80:10:300;

% Size of data chunks to use for testing described above
datasizetest = 5e5;
num_peaks_compiled = [];
...
for i = 1:length(estimated_datapoints)
    ...
    rawdata_smr_current_test = ...
        rawdata_smr(test_slice_start:test_slice_end);
    
    % Create a column vector of the time data in the current block being 
    % analyzed 
    rawdata_smr_time_current_test = ...
        rawdata_smr_time(test_slice_start:test_slice_end);
...
% Based on maximum number of peaks detected for each number of estimated
% datapoints, choose the maximum and use it later
...
while(1)
    ...
    test_slice_end = min(length(rawdata_smr), (loop + 1) * datasize);
    rawdata_smr_current = rawdata_smr(test_slice_start:test_slice_end);

    % Create a column vector of the time data in the current block being analyzed 
    rawdata_smr_time_current = rawdata_smr_time(test_slice_start:...
        test_slice_end);

    % Detect peaks from current block
    [datalast, analysis_params_temp, elapsed_time] = S1_PeakAnalysis_time(-rawdata_smr_current', rawdata_smr_time_current, datafull, loop, analysisparams, estimated_datapoints_best, elapsed_time);
    write_param = 1;
...
% cd to directory with SMR data and save as txt
