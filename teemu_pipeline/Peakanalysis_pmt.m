...
fprintf('number of data points bigger than cutoff is : %1.0f -------', length(idxon)); disp(' ');

if length(idxon) > med_filt_wd_offset
...
Data.normalized_1 = Data.read_1 - median(Data.read_1);
Data.normalized_2 = Data.read_2 - median(Data.read_2);
Data.normalized_3 = Data.read_3 - median(Data.read_3);
      ...
      while (exitflag ~=1)
       ...
       elseif length(otherch)==1 %there is two channles that passed the peak threshold
         ...
         if isempty(idx)==1
           ...
           plotrange = [max(tempstart-150,1):min(tempend+150, xdata(end))];
           ...
           local_baseline3 = median(Data.filtered_3(setxor(plotrange, maxrange)));
            ...
            Peak.baseline1(i) = local_baseline1 + median(Data.read_1); %get the baseline near the peak. added 09/13/2016
            ...
            Peak.baseline2(i) = local_baseline2 + median(Data.read_2);
            ...
            Peak.baseline3(i) = local_baseline3 + median(Data.read_3);
...
%         subplot(3,4,[1 3]); plot(xdata, smooth(Data.filtered_1,3), 'k-');
        subplot(3,4,[1 3]); plot(xdata, Data.filtered_13, 'r-');hold on;
        subplot(3,4,[1 3]); plot(Peak.location1, Data.filtered_13(Peak.location1), '*g');
        subplot(3,4,[1 3]); plot(xdata, Process.peak_threshold1, 'k'); 
        subplot(3,4,[1 3]); plot(xdata, baseline1, 'LineStyle', ':'); hold off;
%         ylim([-0.02 0.2]);
        
%         subplot(3,4,[5 7]); plot(xdata, Data.filtered_22, 'b-'); hold on;
%         subplot(3,4,[5 7]); plot(xdata, smooth(Data.filtered_2,3), 'k-');
        subplot(3,4,[5 7]); plot(xdata, Data.filtered_23, 'r-'); hold on;
        subplot(3,4,[5 7]); plot(Peak.location2, Data.filtered_23(Peak.location2), '*g');
        subplot(3,4,[5 7]); plot(xdata, Process.peak_threshold2, 'k'); hold off;
%          ylim([-0.02 0.2]);
        
%         subplot(3,4,[9 11]); plot(xdata, Data.filtered_32, 'b-'); hold on;
%         subplot(3,4,[9 11]); plot(xdata, smooth(Data.filtered_3,3), 'k-');
        subplot(3,4,[9 11]); plot(xdata, Data.filtered_33, 'r-'); hold on;
        subplot(3,4,[9 11]); plot(Peak.location3, Data.filtered_33(Peak.location3), '*g');
        subplot(3,4,[9 11]); plot(xdata, Process.peak_threshold3, 'k'); 
        subplot(3,4,[9 11]); plot(xdata, baseline3, 'LineStyle', ':');  hold off;
%          ylim([-0.02 0.05]);
        drawnow;
%         reviewparam = input('review single peaks? (yes ==1, No and jump to next section ==2) ');
%        
%        
%         if reviewparam == 1
%             filtrepeat =1;
%             while(filtrepeat ==1)
%                 windowSize = input('type windowsize for moving average filter');
%                 b = (1/windowSize)*ones(1,windowSize);
%                 a=1;
%                 
%                 %now review the peak location and heights
%                 for i=1:Peak.count1
%                     xrange1 = [max(Peak.start1(i)-100,1):min(Peak.end1(i)+100, xdata(end))];
%                     maxrange1 = [max(Peak.start1(i)-50,1):min(Peak.end1(i)+50, xdata(end))];
%                     %                     subplot(3,4,[1 3]); hold off;
%                     %                     plot(xdata, Data.filtered_13, 'b-'); hold on;
%                     %                     plot(Peak.location1(i), Data.filtered_13(Peak.location1(i)), '*g', 'markersize', 10);
%                     %                     hold off;
%                     
%                     subplot(3,4,[4, 8]); plot(xrange1, Data.filtered_13(xrange1), '.', 'color', 'blue'); hold on;
%                     subplot(3,4,[4, 8]); plot(Peak.location1(i), Data.filtered_13(Peak.location1(i)), 'ok');
%                     subplot(3,4,[4, 8]); plot(maxrange1, Data.filtered_12(maxrange1), '-b');
%                     subplot(3,4,[4, 8]); plot(maxrange1, Data.filtered_1(maxrange1), '-k');
%                     subplot(3,4,[4, 8]); plot(maxrange1, Data.filtered_13(maxrange1), '-r');
%                     hold off;
%                     
%                     tempamplitude1 = [tempamplitude1 Peak.amplitude1(i)];
%                     subplot(3,4,12); hist(tempamplitude1);
%                     
%                     input('go?');
%                     
%                 end
%                 filtrepeat = input('repeat again? ' );
%                 
%             end
%             
...
%         else
            ...
            %tempamplitude1= [tempamplitude1 Peak.amplitude1(i)];
            %plot individual peaks, with their height adjusted relative to
            %local baseline.
...
%                             plot(maxrange, Data.filtered_12(maxrange)-baseline1, '-b');
                            ...
                            plot(plotrange, zeros(1,length(plotrange)), 'k-');  drawnow; hold off;
...
data1=[]; data2=[]; data3=[];
display('skipping this segment... too short for peak selection')
input('go?')
end
