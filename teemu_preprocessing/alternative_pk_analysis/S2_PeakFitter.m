...
set(gca,'YLim',[min(freqdata) - 10 max(freqdata) + 10]);
end
...
for i = 1:length(peaks)
    ...
    subplot(2,2,3); plot((-pk_fit_wd:pk_fit_wd) + xdata(peaks(i)), peakfit, 'r')                 
...
for i = 1:length(antipeaks)
    ...
    subplot(2,2,3); plot((-pk_fit_wd:pk_fit_wd) + xdata(antipeaks(i)), antipeakfit, 'r')          
