...
%% Setup
dens_pct_err_cells = compile_fn_handles(@(st, idx) st.dens_pct_err(idx), num_particle_sz);
dens_pct_err_cells{length(dens_pct_err_cells) + 1} = @(st) mean(st.dens_pct_err);

abs_dens_pct_err_cells = compile_fn_handles(@(st, idx) abs(st.dens_pct_err(idx)), num_particle_sz);
abs_dens_pct_err_cells{length(abs_dens_pct_err_cells) + 1} = @(st) mean(abs(st.dens_pct_err));

dens_cv_cells = compile_fn_handles(@(st, idx) st.dens_cv(idx), num_particle_sz);
dens_cv_cells{length(dens_cv_cells) + 1} = @(st) mean(st.dens_cv);

vol_pct_err_cells = compile_fn_handles(@(st, idx) st.vol_pct_err(idx), num_particle_sz);
vol_pct_err_cells{length(vol_pct_err_cells) + 1} = @(st) mean(st.vol_pct_err);

abs_vol_pct_err_cells = compile_fn_handles(@(st, idx) abs(st.vol_pct_err(idx)), num_particle_sz);
abs_vol_pct_err_cells{length(abs_vol_pct_err_cells) + 1} = @(st) mean(abs(st.vol_pct_err));

vol_cv_cells = compile_fn_handles(@(st, idx) st.vol_cv(idx), num_particle_sz);
vol_cv_cells{length(vol_cv_cells) + 1} = @(st) mean(st.vol_cv);

dv_slope_cells = compile_fn_handles(@(st, idx) st.dv_slope(idx), num_particle_sz);
dv_slope_cells{length(dv_slope_cells) + 1} = @(st) mean(st.dv_slope);

norm_dv_slope_cells = compile_fn_handles(@(st, idx) st.dv_slope_norm_to_vol(idx), num_particle_sz);
norm_dv_slope_cells{length(norm_dv_slope_cells) + 1} = @(st) mean(st.dv_slope_norm_to_vol);

szdep_drift_cells = {@(st) get_szdepend_density(st.vol_mean, st.dens_pct_err), ...
    @(st) get_szdepend_volume(st.vol_mean, st.vol_pct_err)};

compiled_handle_cells = {...
    dens_pct_err_cells, abs_dens_pct_err_cells, abs_dens_pct_err_cells, ...
    vol_pct_err_cells, abs_vol_pct_err_cells, vol_cv_cells, ...
    dv_slope_cells, norm_dv_slope_cells, ...
    szdep_drift_cells};
...
%     @(st) st.vol_cv(2),...
%     @(st) st.vol_cv(3),...
%     @(st) get_szdepend_density(vol_mean, dens_pct_error),...
%     @(st) get_szdepend_volume(vol_mean, vol_pct_error)};

size_label = ["15um", "12um", "10um", "9um", "8um", "7um", "6um", "5um", "Mean"];
quant_label = ["density_pct_error", "abs_density_pct_error", "density_cv", ...
    "volume_pct_error", "abs_volume_pct_error", "volume_cv", ...
    "local_dv_slope", "norm_local_dv_slope", ...
    "global_dv_slope"];
...
function slope = get_szdepend_density(vol_mean, dens_pct_error)
    p = polyfit(vol_mean, dens_pct_error, 1);
    slope = p(1);
end

function slope = get_szdepend_volume(vol_mean, vol_pct_error)
    p = polyfit(vol_mean, vol_pct_error, 1);
    slope = p(1);
end

function cell_handles = compile_fn_handles(fn_handle_base, num_particle_sz)
cell_handles = cell(num_particle_sz, 1);
for i = 1:length(num_particle_sz)
    cell_handles{i, 1} = @(st) fn_handle_base(st, i);
end
end