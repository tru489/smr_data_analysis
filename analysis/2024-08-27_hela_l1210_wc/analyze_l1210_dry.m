...
%% Load data
h2o_wnk_paths = ls("l1210_data\h2o_wnk");
d2o_wnk_paths = ls("l1210_data\d2o_wnk");
h2o_dmso_paths = ls("l1210_data\h2o_dmso");
d2o_dmso_paths = ls("l1210_data\d2o_dmso");
...
h2o_wnk_thresh = [22 200];
d2o_wnk_thresh = [12 120];
h2o_dmso_thresh = [22 200];
d2o_dmso_thresh = [12 120];
...
h2o_dens = mean([h2o_gsk_dens_arr, h2o_wnk_dens_arr, h2o_dmso_dens_arr]);
...
d2o_dens = mean([d2o_gsk_dens_arr, d2o_wnk_dens_arr, d2o_dmso_dens_arr]);

%% Get bms from threshold
h2o_wnk_bm = get_mean_bms_and_save(h2o_wnk_paths, h2o_wnk_thresh, 'l1210_results');
h2o_dmso_bm = get_mean_bms_and_save(h2o_dmso_paths, h2o_dmso_thresh, 'l1210_results');

d2o_wnk_bm = get_mean_bms_and_save(d2o_wnk_paths, d2o_wnk_thresh, 'l1210_results');
d2o_dmso_bm = get_mean_bms_and_save(d2o_dmso_paths, d2o_dmso_thresh, 'l1210_results');
...
dry_tab.total_volume_fl = [743.0, 738.6, 742.4, 1136, 1039, 1077]';
dry_tab.water_content = (dry_tab.total_volume_fl - dry_tab.dry_volume_fl) ./ dry_tab.total_volume_fl;

writetable(dry_tab, 'l1210_results\dry_properties.csv')
