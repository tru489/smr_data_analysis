...
%% Load data
h2o_sts_paths = ls("data\h2o_sts");
d2o_sts_paths = ls("data\d2o_sts");
...
%% Thresholding
% plot_pths_for_thresholding(h2o_sts_paths, 'h2o\_sts')
% plot_pths_for_thresholding(d2o_sts_paths, 'd2o\_sts')
% plot_pths_for_thresholding(h2o_dmso_paths, 'h2o\_dmso')
plot_pths_for_thresholding(d2o_dmso_paths, 'd2o\_dmso')

h2o_sts_thresh = [25 600];
d2o_sts_thresh = [25 600];
h2o_dmso_thresh = [25 600];
d2o_dmso_thresh = [25 600];

%% Density baseline calculation
h2o_trm_dens_arr = get_avg_density(h2o_sts_paths, h2o_sts_thresh, slope, intc, h2o_rf);
...
d2o_trm_dens_arr = get_avg_density(d2o_sts_paths, d2o_sts_thresh, slope, intc, d2o_rf);
...
h2o_sts_bm = get_mean_bms_and_save(h2o_sts_paths, h2o_sts_thresh, 'results');
h2o_dmso_bm = get_mean_bms_and_save(h2o_dmso_paths, h2o_dmso_thresh, 'results');

d2o_sts_bm = get_mean_bms_and_save(d2o_sts_paths, d2o_sts_thresh, 'results');
d2o_dmso_bm = get_mean_bms_and_save(d2o_dmso_paths, d2o_dmso_thresh, 'results');

%% calculate density and volume
[dens_sts, vol_sts] = calc_dry_dens_vol(h2o_sts_bm, d2o_sts_bm, h2o_trm_dens_arr, d2o_trm_dens_arr);
[dens_dmso, vol_dmso] = calc_dry_dens_vol(h2o_dmso_bm, d2o_dmso_bm, h2o_dmso_dens_arr, d2o_dmso_dens_arr);

labels = ["Rep" + string(1:num_replicates) + "_sts" "Rep" + string(1:num_replicates) + "_dmso"];
dry_tab = table();
dry_tab.labels = labels';
dry_tab.dry_density_gcm3 = [dens_sts, dens_dmso]';
dry_tab.dry_volume_fl = [vol_sts, vol_dmso]';
dry_tab.avg_buoyant_mass_h2o = [h2o_sts_bm, h2o_dmso_bm]';
dry_tab.avg_buoyant_mass_d2o = [d2o_sts_bm, d2o_dmso_bm]';
...
dry_tab.total_volume_fl = [2524.8	2867.8	2907.4	2800.4]';
