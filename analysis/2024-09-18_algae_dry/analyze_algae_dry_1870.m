...
h2o_low_paths = ls("1870_data\h2o_low");
h2o_high_paths = ls("1870_data\h2o_high");
d2o_low_paths = ls("1870_data\d2o_low");
d2o_high_paths = ls("1870_data\d2o_high");
...
h2o_low_thresh = [0 inf];
h2o_high_thresh = [0 50];

d2o_low_thresh = [0 inf];
d2o_high_thresh = [0 40];
...
h2o_dens = mean([low_h2o_dens_arr, high_h2o_dens_arr]);
...
d2o_dens = mean([low_d2o_dens_arr, high_d2o_dens_arr]);

h2o_low_bm = get_mean_bms_and_save(h2o_low_paths, h2o_low_thresh, '1870_results');
h2o_high_bm = get_mean_bms_and_save(h2o_high_paths, h2o_high_thresh, '1870_results');
d2o_low_bm = get_mean_bms_and_save(d2o_low_paths, d2o_low_thresh, '1870_results');
d2o_high_bm = get_mean_bms_and_save(d2o_high_paths, d2o_high_thresh, '1870_results');
...
labels = ["Rep" + string(1:2) + "_low" "Rep" + string(1:1) + "_high"];
dry_tab = table();
...
dry_tab.avg_density_h2o_gcm3 = repmat(h2o_dens, 1, 3)';
dry_tab.avg_density_d2o_gcm3 = repmat(d2o_dens, 1, 3)';
writetable(dry_tab, '1870_results\dry_properties.csv')
