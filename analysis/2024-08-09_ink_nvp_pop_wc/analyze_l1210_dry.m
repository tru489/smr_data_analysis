...
h2o_nvp_paths = ls("data\h2o_nvp");
d2o_nvp_paths = ls("data\d2o_nvp");
...
h2o_ink_thresh = [15 120];
d2o_ink_thresh = [10 100];
h2o_nvp_thresh = [15 120];
d2o_nvp_thresh = [10 100];
h2o_dmso_thresh = [15 120];
d2o_dmso_thresh = [10 100];

%% Density baseline calculation
h2o_gsk_dens_arr = get_avg_density(h2o_ink_paths, h2o_ink_thresh, slope, intc, h2o_rf);
...
d2o_gsk_dens_arr = get_avg_density(d2o_ink_paths, d2o_ink_thresh, slope, intc, d2o_rf);
...
h2o_nvp_bm = get_mean_bms_and_save(h2o_nvp_paths, h2o_nvp_thresh, 'results');
...
d2o_nvp_bm = get_mean_bms_and_save(d2o_nvp_paths, d2o_nvp_thresh, 'results');
d2o_dmso_bm = get_mean_bms_and_save(d2o_dmso_paths, d2o_dmso_thresh, 'results');

%% calculate density and volume
[dens_ink, vol_ink] = calc_dry_dens_vol(h2o_ink_bm, d2o_ink_bm, h2o_gsk_dens_arr, d2o_gsk_dens_arr);
...
labels = ["Rep" + string(1:num_replicates) + "_ink" "Rep" + string(1:num_replicates) + "_nvp" "Rep" + string(1:num_replicates) + "_dmso"];
dry_tab = table();
dry_tab.labels = labels';
dry_tab.dry_density_gcm3 = [dens_ink, dens_nvp, dens_dmso]';
dry_tab.dry_volume_fl = [vol_ink, vol_nvp, vol_dmso]';
dry_tab.avg_buoyant_mass_h2o = [h2o_ink_bm, h2o_nvp_bm, h2o_dmso_bm]';
dry_tab.avg_buoyant_mass_d2o = [d2o_ink_bm, d2o_nvp_bm, d2o_dmso_bm]';
...
dry_tab.total_volume_fl = [711.0, 703.2, 707.2, 663.3, 667.2, 669.7, 961.4, 999.0, 1012]';
