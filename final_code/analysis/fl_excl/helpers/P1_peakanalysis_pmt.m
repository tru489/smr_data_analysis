function [readout_pmt, progress_msg, pass_struct_pmt] = ...
    P1_peakanalysis_pmt(run_params, analysis_params, pass_struct_pmt, ...
    n_segment, num_segments, rawdata_pmt, rawdata_time_pmt, disp_params)
% First stage of PMT peak detection for a given segment of PMT data from
% all PMTS
%
% Arguments:
%   run_params (struct): running params for analysis
%   pass_struct_pmt (struct): essential parameters necessary to pass
%       through each iteration of analysis. Includes elapsed_time,
%       elapsed_index, and elapsed_peak_count
%   n_segment (int): segment number of PMT data
%   num_segments (int): total number of segments in PMT data
%   rawdata_pmt (array(double)): raw data from all PMTs
%   rawdata_time_pmt (array(double)): raw time data for PMTs
%   disp_params (struct): display parameters for script
% Returns:
%   readout_pmt (table): table containing relevant peak parameters
%   progress_msg (str): message to be displayed in progress dialog
%   pass_struct_pmt (struct): essential  (modified) parameters necessary to pass
%       through each iteration of analysis

elapsed_time = pass_struct_pmt.elapsed_time;
elapsed_index = pass_struct_pmt.elapsed_index;
elapsed_peak_count = pass_struct_pmt.elapsed_peak_count;

% Allocating peak detection parameters
n_pmt_channel = run_params.fl_excl.n_pmt_channel;
analysis_mode = disp_params.analysis_mode;
disp_progress = disp_params.disp_progress;
...
if length(idx_rangeofinterest) < med_filt_window_size
    ...
    pass_struct_pmt.elapsed_time = elapsed_time + ...
        (rawdata_time_pmt(end) - rawdata_time_pmt(1));
    pass_struct_pmt.elapsed_index = elapsed_index + ...
        length(rawdata_pmt{1});
    return;
end
...
%The median normalization step is skipped to make sure fluorescence exclusion 
% baseline does not get erased
% Data.normalized = cellfun(@(x) x-median(x),Data.read,...
%     'UniformOutput',false);
Data.normalized = Data.read; 

% Apply median filter then moving-average filter
Data.filtered_med = cellfun(@(x) medfilt1(x,med_filt_length), ...
    Data.normalized, 'UniformOutput', false);
...
% baselines
for i = 1:n_pmt_channel
    Process.baseline{i}=interp1(med_filt_x_axis_Ind, med_filt_pmt_base(i,:), ...
        x_axis_Ind, 'linear', 'extrap');
end
...
% peak
Process.peak_indices = cellfun(@(x,y) find(x > y), Data.filtered_med, ...
    Process.peak_threshold,"UniformOutput",false);
if fxm_mode == 1
    Process.peak_indices{fxm_channel} = ...
        find(Data.filtered_med{fxm_channel} < ...
        Process.peak_threshold{fxm_channel});
end
...
if sum([Peak.count{:}])==0
    ...
    readout_pmt = [];
    progress_msg = [];
    pass_struct_pmt.elapsed_time = elapsed_time + (rawdata_time_pmt(end) - rawdata_time_pmt(1));
    pass_struct_pmt.elapsed_index = elapsed_index + length(rawdata_pmt{1,i});
    return;
end
...
pass_struct_pmt.elapsed_time = elapsed_time + (rawdata_time_pmt(end) - rawdata_time_pmt(1));
pass_struct_pmt.elapsed_index = elapsed_index + length(rawdata_pmt{1});
pass_struct_pmt.elapsed_peak_count = elapsed_peak_count + length(Peak.time_of_detection);
