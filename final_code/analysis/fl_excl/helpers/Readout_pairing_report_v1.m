function Readout_pairing_report_v1(report_dir, smr_input, pmt_input, analysis_params,trace,window,min_time_threshold,max_time_threshold,readout_paired,multiplet_count)
...
rpt = Report(fullfile(report_dir, append("Readout_pairing_report_", time_stamp)),'pdf');
open(rpt)
...
dot_size = 8;
tstamp_fig = figure(...
    'OuterPosition', [0 0.5*scrsize(4) 0.95*scrsize(3) 0.3*scrsize(4)]);
scatter((1/60)*(smr_input.real_time_sec-smr_input.real_time_sec(1)), ...
    ones(size(smr_input.real_time_sec)),dot_size,'filled','r')
hold on
scatter((1/60)*(pmt_input.real_time_sec-smr_input.real_time_sec(1)), ...
    -1*ones(size(pmt_input.real_time_sec)),dot_size,'filled','b')
ylim([-2,2])
title('Real time SMR vs PMT timestamps')
...
%     subplot(2,n_pmt_channel,i);
        ...
        title(append('', ' ', rpt_title_color_lab(i), ' vs BM'))
...
%         caxis([prctile(color_overlay,10) prctile(color_overlay,90)]);
        title(append('', ' ', 'Transit time', ' vs BM'))
