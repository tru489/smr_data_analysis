function stats_cell = get_water_content_stats(run_params, ...
    ...
    detected_smr_peaks, output_pmt_table)
...
% Forward/backward peak detection
...
stats_cell{i} = ...
    strcat("Number of forward peaks: ", num_fwd);
i = i + 1;
stats_cell{i} = ...
...
stats_cell{i} = ...
    strcat("Number of reverse peaks: ", num_back);
i = i + 1;
stats_cell{i} = ...
    {sprintf("Pair rate of reverse peaks: %0.3f%%", 100 *  num_back / num_pair_peaks)};
i = i + 1;

stats_cell{i} = ...
    strcat("Number of forward and reverse peaks paired: ", height(paired_datasmr));
i = i + 1;
