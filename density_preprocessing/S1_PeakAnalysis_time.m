function data = S1_PeakAnalysis_time(x, s, vv, datalast, sectionnumber, analysisparam)
...
diff_threshold = 0.05;      % Find extremely flat part of curve (sys1)
med_filt_wd = 30;           % window of median filter, which removes the flat part in the anti-node
bs_dev_thres = 0.5;         % threshold used to remove the flat part in the anti-node
unqPeakDist = 300;          % distance over which is a unique 2nd mode peaks
offset_input = 20;          % baseline offset to select for peaks


% added 01062021 - compensate for number of data points & noise leve;
estimated_datapoints = 500; %for full transit; deafult
estimated_noise = 0.5; %Hz
...
idx_in=find(ydata(idx)<3000);  %change from 3000 to 1500 (01052021) 
idx=idx(idx_in);
...
segment_threshold = segment_threshold*estimated_datapoints/400;  % add on 01052020
sidelength = 2000; %number of more indices on each side of baseline 
