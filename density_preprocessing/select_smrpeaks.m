...
figure('OuterPosition',[0.3*scrsize(3) 0.35*scrsize(4) 0.3*scrsize(3) 0.65*scrsize(4)])
...
%             save=[sampletime(Peak.start(i):Peak.start(i+1)-4); samplepeak(Peak.start(i):Peak.start(i+1)-4)];
            tempidx=find(datasmr(:,15)==Peak.sectnum(i) & datasmr(:,18)==Peak.peakorder(i));
            if tempidx == i
...
writematrix(datasmr_processed, [save_dir filesep fname '.csv'])
