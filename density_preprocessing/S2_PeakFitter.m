function [pkidx_poly, pkht_poly, apkidx_poly, apkht_poly, quadbase2, htdiff_poly, ahtdiff_poly] = S2_PeakFitter(xdata, ydata, baseline, peaks, peakwidth, dispprogress)
...
quadbaseline = polyfit(xbasedata, ybasedata,2);
quadbase2 = quadbaseline(1);
quadbase1 = quadbaseline(2);
% 
% linbaseline = polyfit(xbasedata, ybasedata, 1);
% baselineslope = linbaseline(1);

baselinefreq = (polyval(quadbaseline, xdata))';
median(ydata);
...
if dispprogress == 1
    subplot(2,2,3); cla
    hold off; plot(xdata, ydata, '-'); hold on; plot(xdata, baselinefreq); 
    % input('go');
    hold off;
...
for i = 1:length(peaks)
    if length(peaks)==3 && i==2
        pk_fit_wd = round(peakwidth/45);
    else
        pk_fit_wd = round(peakwidth/30); 
    end
    pk_fit_segx = (max((xdata(peaks(i)) - pk_fit_wd), 1):min((xdata(peaks(i)) + pk_fit_wd),xdata(end)))';                 % actual segment of peak to be fitted
    ...
    subplot(2,2,3); plot((1:length(pk_fit_segx)) - pk_fit_wd-1 + xdata(peaks(i)), peakfit, 'r')                 
    subplot(2,2,3); plot(pkidx_poly(i), pky, 'or')    
    end
end

for i = 1:length(antipeaks)
    antipk_fit_segx = (max((xdata(antipeaks(i)) - pk_fit_wd), 1):min((xdata(antipeaks(i)) + pk_fit_wd), xdata(end)))';     % actual segment of peak to be fitted
...
if dispprogress ==1
    subplot(2,2,3); plot([1:length(antipk_fit_segx)] - pk_fit_wd-1 + xdata(antipeaks(i)), antipeakfit, 'r')          
