function [pkidx_poly, pkht_poly, apkidx_poly, apkht_poly, ...
    baselineslope, htdiff_poly, ahtdiff_poly] = S2_PeakFitter(xdata, ...
    ydata, baseline, peaks, peakwidth, dispprogress)

%%% Original by Sungmin Son
%%% Modified by Nikita Khlystov
%%% Latest edit on 12/16

% Baseline indices and corresponding y values
xbasedata = baseline;
ybasedata = ydata(baseline)';

% Find maximum values between adjacent peaks (antipeaks; correspond with 
% nodes)
...
% Linearly fit the baseline slope
linbaseline = polyfit(xbasedata, ybasedata, 1);
baselineslope = linbaseline(1);

% Based on regression, provide linear interpolation of baseline
baselinefreq = (polyval(linbaseline, xdata))';
median(ydata);

% Subtract linear baseline fit from data
freqdata = ydata' - baselinefreq; 

% Subtract linear fit of baseline data from observed baseline fragments
...
% Find points to the left and right of the FWHM of the first and last peak
% respectively
hahtwd_ll = find(freqsmth <= freqsmth(peaks(1))/2, 1);
hahtwd_rr = find(freqsmth <= freqsmth(peaks(end))/2, 1, 'last');

% Plot peaks and antipeaks (red) and points to the left and right of the
% FWHM of the first and last peaks (to give an idea of where the peak is in
% the whole freq signal segment; green)
if dispprogress ==1
...
for i = 1:length(peaks)
    % Slice out a segment of the peak to be fitted
...
% Find mean node deviation from polynomial fits
